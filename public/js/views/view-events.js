(function () {
  ...
  async function mount(outlet) {
    ...
    try {
      if (!cachedHtml) {
        var resp = await fetch('/pages/events/event.html', { credentials: 'include' });
        if (!resp.ok) throw new Error('HTTP ' + resp.status);
        cachedHtml = await resp.text();
      }
      var doc = new DOMParser().parseFromString(cachedHtml, 'text/html');
      var content = doc.querySelector('.events-col') || doc.querySelector('main') || doc.body;
      ...
      outlet.innerHTML = '<div class="spa-error"><i class="fa-regular fa-circle-xmark"></i><p>Could not load events.</p></div>';
    }
  }
  ...
  if (window.SpaRouter) {
    window.SpaRouter.register('events', { mount: mount, unmount: unmount });
  }
})();
